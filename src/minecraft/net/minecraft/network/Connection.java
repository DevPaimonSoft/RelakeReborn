...
public class Connection extends SimpleChannelInboundHandler<Packet<?>> {
    ...
    PacketEvent receive = new PacketEvent(null, PacketEvent.Type.RECEIVE);
    protected void channelRead0(ChannelHandlerContext pContext, Packet<?> pPacket) {
        if (this.channel.isOpen()) {
            PacketListener packetlistener = this.packetListener;
            receive.setPacket(pPacket);
            Main.getInstance().getEventHandler().post(receive);
    ...
    PacketEvent send = new PacketEvent(null, PacketEvent.Type.SEND);

    public void send(Packet<?> pPacket, @Nullable PacketSendListener pSendListener) {
        send.setPacket(pPacket);
        Main.getInstance().getEventHandler().post(send);
        if (send.isCancelled()) {
            LOGGER.debug("Cancelled packet: {}", pPacket.getClass().getSimpleName());
            send.intercept();
            return;
        }
