...
public class AnimationUtils {
    public static void animateCrossbowHold(ModelPart pRightArm, ModelPart pLeftArm, ModelPart pHead, boolean pRightHanded) {
        ...
        modelpart.xRot = (float) (-Math.PI / 2) + pHead.xRot + 0.1F;
        modelpart1.xRot = -1.5F + pHead.xRot;
    }

    public static void animateCrossbowCharge(ModelPart pRightArm, ModelPart pLeftArm, float pMaxCrossbowChargeDuration, int pTicksUsingItem, boolean pIsRightHand) {
        ModelPart modelpart = pIsRightHand ? pRightArm : pLeftArm;
        ModelPart modelpart1 = pIsRightHand ? pLeftArm : pRightArm;
        modelpart.yRot = pIsRightHand ? -0.8F : 0.8F;
        modelpart.xRot = -0.97079635F;
        modelpart1.xRot = modelpart.xRot;
        float f = Mth.clamp((float)pTicksUsingItem, 0.0F, pMaxCrossbowChargeDuration);
        float f1 = f / pMaxCrossbowChargeDuration;
        modelpart1.yRot = Mth.lerp(f1, 0.4F, 0.85F) * (float)(pIsRightHand ? 1 : -1);
    ...
    public static void swingWeaponDown(ModelPart pRightArm, ModelPart pLeftArm, HumanoidArm pMainArm, float pAttackTime, float pAgeInTicks) {
        ...
        pRightArm.zRot = 0.0F;
        pLeftArm.zRot = 0.0F;
        pRightArm.yRot = (float) (Math.PI / 20);
        pLeftArm.yRot = (float) (-Math.PI / 20);
        if (pMainArm == HumanoidArm.RIGHT) {
    ...
    public static void bobModelPart(ModelPart pModelPart, float pAgeInTicks, float pMultiplier) {
        pModelPart.zRot = pModelPart.zRot + pMultiplier * (Mth.cos(pAgeInTicks * 0.09F) * 0.05F + 0.05F);
        pModelPart.xRot = pModelPart.xRot + pMultiplier * Mth.sin(pAgeInTicks * 0.067F) * 0.05F;
    }
