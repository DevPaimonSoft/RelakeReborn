...
public class SilverfishModel extends EntityModel<EntityRenderState> {
    ...
    @Override
    public void setupAnim(EntityRenderState p_368765_) {
        super.setupAnim(p_368765_);

        for (int i = 0; i < this.bodyParts.length; i++) {
            this.bodyParts[i].yRot = Mth.cos(p_368765_.ageInTicks * 0.9F + (float)i * 0.15F * (float) Math.PI)
                * (float) Math.PI
                * 0.05F
                * (float)(1 + Math.abs(i - 2));
            this.bodyParts[i].x = Mth.sin(p_368765_.ageInTicks * 0.9F + (float)i * 0.15F * (float) Math.PI)
                * (float) Math.PI
                * 0.2F
                * (float)Math.abs(i - 2);
        }
