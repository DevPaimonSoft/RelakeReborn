...
import su.relake.client.api.context.implement.PostMotionEvent;
...
public class LocalPlayer extends AbstractClientPlayer {
    ...
    UpdateEvent eventUpdate = new UpdateEvent();
    ...
    public void tick() {
        ...
        if (this.hasClientLoaded()) {
            ...
            Main.getInstance().getEventHandler().post(eventUpdate);
    ...
    private final MotionEvent motionEvent = new MotionEvent(0, 0, 0, 0, 0, false, null);
    ...
    private void sendPosition() {
        ...
        if (this.isControlledCamera()) {
            ...
            Main.getInstance().getEventHandler().post(motionEvent);
            if (motionEvent.isCancelled()) {
                motionEvent.intercept();
                return;
            }
            double d4 = motionEvent.getX() - this.xLast;
            double d0 = motionEvent.getY() - this.yLast;
            double d1 = motionEvent.getZ() - this.zLast;
            double d2 = motionEvent.getYaw() - this.yRotLast;
            double d3 = motionEvent.getPitch() - this.xRotLast;
...
//            this.positionReminder = 0;
            ...
            Main.getInstance().getEventHandler().post(new PostMotionEvent());
    ...
    public void aiStep() {
        ...
        boolean state = Main.getInstance().getModBus().noSlowModProcessor.isState();

        if (this.isUsingItem() && !this.isPassenger() && !state) {
        ...
        if (abilities.flying && this.isControlledCamera()) {
            ...
            if (j != 0) {
                this.setDeltaMovement(this.getDeltaMovement().add(0.0, (float) j * abilities.getFlyingSpeed() * 3.0F, 0.0));
            }
        }
