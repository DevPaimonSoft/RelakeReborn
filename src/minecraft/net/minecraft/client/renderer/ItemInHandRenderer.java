...
public class ItemInHandRenderer {
    ...
    public void renderItem(
        ...
        boolean flag = (pEntity.getMainArm() == HumanoidArm.LEFT) != pLeftHand;
    ...
    private void renderOneHandedMap(
        ...
        float f1 = sqrt(pSwingProgress);
        float f2 = sin(f1 * (float) Math.PI);
        float f3 = -0.5F * f2;
        float f4 = 0.4F * sin(f1 * (float) (Math.PI * 2));
        float f5 = -0.3F * sin(pSwingProgress * (float) Math.PI);
    ...
    private void renderTwoHandedMap(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, float pPitch, float pEquippedProgress, float pSwingProgress) {
        float f = sqrt(pSwingProgress);
        float f1 = -0.2F * sin(pSwingProgress * (float) Math.PI);
        float f2 = -0.4F * sin(f * (float) Math.PI);
    ...
    private void renderPlayerArm(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, float pEquippedProgress, float pSwingProgress, HumanoidArm pSide) {
        ...
        float f1 = sqrt(pSwingProgress);
        float f2 = -0.3F * sin(f1 * (float) Math.PI);
        float f3 = 0.4F * sin(f1 * (float) (Math.PI * 2));
        float f4 = -0.4F * sin(pSwingProgress * (float) Math.PI);
        ...
        float f5 = sin(pSwingProgress * pSwingProgress * (float) Math.PI);
        float f6 = sin(f1 * (float) Math.PI);
    ...
    private void applyItemArmAttackTransform(PoseStack pPoseStack, HumanoidArm pHand, float pSwingProgress) {
        ...
        float f = sin(pSwingProgress * pSwingProgress * (float) Math.PI);
        pPoseStack.mulPose(Axis.YP.rotationDegrees((float) i * (45.0F + f * -20.0F)));
        float f1 = sin(sqrt(pSwingProgress) * (float) Math.PI);
            ...
            if (!pPlayer.isScoping()) {
                ...
                if (pStack.isEmpty()) {
                    ...
                    if (!IClientItemExtensions.of(pStack)
                        ...
                        } else if (Main.getInstance().getModBus().swingAnimationModProcessor.isState()) {
                            int k = flag1 ? 1 : -1;
                            if (k == 1) {
                                final var swingAnimationsMod = Main.getInstance().getModBus().swingAnimationModProcessor;
                                switch (swingAnimationsMod.mode.getValue()) {
                                    case "1" -> {
                                        pPoseStack.translate(0.56f, -0.25f, -0.71999997f);
                                        pPoseStack.translate(0, -0, 0);

                                        pPoseStack.last().pose().rotate(new Quaternionf().fromAxisAngleDeg(new Vector3f(0, 1, 0), 60));
                                        pPoseStack.last().pose().rotate(new Quaternionf().fromAxisAngleDeg(new Vector3f(1, 0, 0), -90));
                                        pPoseStack.last().pose().rotate(new Quaternionf().fromAxisAngleDeg(new Vector3f(0, 1, 0), 60));

                                        float var4 = sin(sqrt(pSwingProgress) * (float) Math.PI);
                                        pPoseStack.last().pose().rotate(new Quaternionf().fromAxisAngleDeg(new Vector3f(1, 0, 0), var4 * -60f));

                                    }
                                    case "2" -> {
                                        float f = sin(pSwingProgress * (PI / 2.0f) * 2.0f);
                                        float f2 = 40f;
                                        float f4 = f2 / 10.0f / 10.0f * 40.0f;
                                        pPoseStack.translate(.95f, -0.5f, -1.075f);
                                        pPoseStack.translate(0, -0, 0);

                                        pPoseStack.last().pose().rotate(Axis.YP.rotationDegrees(1 * 45.0f));
                                        pPoseStack.last().pose().rotate(Axis.YP.rotationDegrees(1 * f * -(f4 / 2.0f)));
                                        pPoseStack.last().pose().rotate(Axis.ZP.rotationDegrees(1 * f * -(f4 / 2.0f)));
                                        pPoseStack.last().pose().rotate(Axis.XP.rotationDegrees(1 * f * -f4));

                                        pPoseStack.translate((1 * -0.5f), 0.2f, 0.0);

                                        pPoseStack.last().pose().rotate(Axis.YP.rotationDegrees(30));
                                        pPoseStack.last().pose().rotate(Axis.XP.rotationDegrees(-80));
                                        pPoseStack.last().pose().rotate(Axis.YP.rotationDegrees(60));
                                    }
                                    case "3" -> {
                                        pPoseStack.translate(.95f, -0.5f, -1.075f);
                                        pPoseStack.translate(0, -0, 0);

                                        float anim = (float) Math.sin(pSwingProgress * (Math.PI / 2) * 2);

                                        pPoseStack.last().pose().rotate(new Quaternionf().fromAxisAngleDeg(new Vector3f(0, 1, 0), 0));
                                        pPoseStack.last().pose().rotate(new Quaternionf().fromAxisAngleDeg(new Vector3f(0, 0, 1), 0));
                                        pPoseStack.last().pose().rotate(new Quaternionf().fromAxisAngleDeg(new Vector3f(1, 0, 0), -80 * anim));
                                    ...
                                    case "4" -> {
                                        ...
                                        pPoseStack.last().pose().rotate(new Quaternionf().fromAxisAngleDeg(new Vector3f(0, 1, 0), 90));
                                        pPoseStack.last().pose().rotate(new Quaternionf().fromAxisAngleDeg(new Vector3f(0, 0, 1), -60));
                                        pPoseStack.last().pose().rotate(new Quaternionf().fromAxisAngleDeg(new Vector3f(1, 0, 0), -80 - 40 * anim));

                                    }
                                }

                                //scale
                                final var handScale = swingAnimationsMod.handScale.getValue();
                                pPoseStack.scale(handScale,
                                        handScale,
                                        handScale
                                );
                            } else {
                                float f6 = -0.4F * sin(sqrt(pSwingProgress) * (float) Math.PI);
                                float f = 0.2F * sin(sqrt(pSwingProgress) * ((float) Math.PI * 2F));
                                float f1 = -0.2F * sin(pSwingProgress * (float) Math.PI);
                                int j = flag1 ? 1 : -1;
    ...
    private void swingArm(float pSwingProgress, float pEquippedProgress, PoseStack pPoseStack, int pDirection, HumanoidArm pArm) {
        float f = -0.4F * sin(sqrt(pSwingProgress) * (float) Math.PI);
        float f1 = 0.2F * sin(sqrt(pSwingProgress) * (float) (Math.PI * 2));
        float f2 = -0.2F * sin(pSwingProgress * (float) Math.PI);
    ...
    private boolean shouldInstantlyReplaceVisibleItem(ItemStack pOldItem, ItemStack pNewItem) {
        return ItemStack.matches(pOldItem, pNewItem) || !this.itemModelResolver.shouldPlaySwapAnimation(pNewItem);
    }
    ...
    enum HandRenderSelection {
        ...
        HandRenderSelection(final boolean pRenderMainHand, final boolean pRenderOffHand) {
