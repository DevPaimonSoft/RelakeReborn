...
public class KeyboardHandler {
    ...
    private static final boolean chunkDebugKeys = Boolean.getBoolean("chunk.debug.keys");
    ...
    public void keyPress(long pWindowPointer, int pKey, int pScanCode, int pAction, int pModifiers) {
        if (pWindowPointer == this.minecraft.getWindow().getWindow()) {
            ...
            boolean flag6;
            flag6 = flag5 || this.minecraft.screen instanceof PauseScreen pausescreen && !pausescreen.showsPauseMenu();

            if (pAction == 0) {
                KeyMapping.set(inputconstants$key, false);

                KeyPressEvent keyPressContext = new KeyPressEvent(pKey);

                keyPressContext.intercept();
