...
public enum Unit {
    ...
    public static Unit getLargest(long pBytes) {
        if (pBytes < 1024L) {
            ...
            try {
                int i = (int)(Math.log((double)pBytes) / Math.log(1024.0));
    ...
    public static double convertTo(long pBytes, Unit pUnit) {
        return pUnit == B ? (double)pBytes : (double)pBytes / Math.pow(1024.0, (double)pUnit.ordinal());
    }
    ...
    public static String humanReadable(long pBytes) {
        ...
        if (pBytes < 1024L) {
            ...
            int j = (int)(Math.log((double)pBytes) / Math.log(1024.0));
            String s = "KMGTPE".charAt(j - 1) + "";
            return String.format(Locale.ROOT, "%.1f %sB", (double)pBytes / Math.pow(1024.0, (double)j), s);
        }
    }
